...
portable 档仍必须通过 OOXML 完整性、OfficeCLI `validate`、inspect blocking findings 清零与视觉门。有可信对象映射时执行 OfficeCLI 改字/移动持久化探针；postflight 无可信对象映射时记录 `not_automated` warning，交付声明不得声称探针已完成。有 LibreOffice+pdftoppm 时产出逐页渲染；否则需用户视觉豁免。portable 交付状态至少为 `pass_with_warnings`，并写明未执行 PowerPoint 原生门禁。
...
macOS auto/native 运行目录不得位于 `/private/tmp`；显式 portable 档可使用隔离临时目录。`inspect` 阶段不启动 Chrome/Chromium；OfficeCLI HTML preview 只记为 `deferred_to_powerpoint`，不能替代 native 档的 PowerPoint 事实渲染，但在 portable 档可由 LibreOffice 渲染补齐。
...
`render_manifest.json` 原子绑定当前 PPTX SHA、验证档位、渲染器、PDF、contact sheet 和每页 PNG 的路径与 SHA。`visual_review.json` 再绑定 render manifest SHA；事实源可以是 PowerPoint 或 portable 档的 LibreOffice。三件套不完整、文件被改动、档位不符或视觉拒绝均不能 finalize。
