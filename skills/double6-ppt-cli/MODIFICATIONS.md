...
The integration is implemented outside the vendor tree:

- a stable semantic manifest and fail-closed object mapper;
...
- findings normalization, dual repair routing and bounded patch ledger;
...
- SHA-bound visual review/waiver policy and Microsoft PowerPoint-first verification;
...
- explicit long AppleEvent timeout, failure cleanup, and semantic-formula fail-closed handling so business formulas cannot be auto-deleted without exact template identity proof.
- cross-slide navigation selected-state detection for postflight, plus template-fill selected-state replay that derives the active section from confirmed section starts and moves only the template's uniquely proven selected/unselected fill and text styles; ambiguous styles fail closed.
